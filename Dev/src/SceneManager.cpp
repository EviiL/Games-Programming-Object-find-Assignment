...
bool SceneManager::LoadScene(std::string pPath) {
	...
	RenderComponent * renderC = new RenderComponent(&m_CurrentScene_->getObjects()[index], "texture", TEXTURE);
	PlayerKeyboardControlComponent * keyComp = new PlayerKeyboardControlComponent(&m_CurrentScene_->getObjects()[index]);
	MeshFactory factory;

	renderC->AttachMesh(factory.create("Models/cube.obj", glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(1.0f, 1.0f, 1.0f), "TEST"));
	renderC->AttachMesh(factory.create("Models/cube.obj", glm::vec3(-5.0f, 0.0f, 1.0f), glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(1.0f, 1.0f, 2.0f), "TEST"));
